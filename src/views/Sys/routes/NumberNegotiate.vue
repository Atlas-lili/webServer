<template>
    <div class="number-negotiate">
        <el-breadcrumb separator-class="el-icon-arrow-right">
            <el-breadcrumb-item :to="{ path: '/Sys/numbernegotiate' }"><i class="el-icon-service"></i>号码管理</el-breadcrumb-item>
            ...
            <el-collapse-item title="筛选" name="1">
                <el-form :inline="true" ref="form" :model="form1" label-position="left" size="mini">
                    <el-form-item label="预占代理商：">
                        <el-select v-model="form1.agent" filterable clearable placeholder="请输入关键字查询">
                            <el-option
                    ...
                    <el-form-item label="操作人：">
                        <el-select v-model="form1.so_op" filterable clearable placeholder="请输入关键字查询">
                            <el-option
                    ...
                    <el-form-item label="协商结果：">
                        <el-select v-model="form1.consultResult" clearable placeholder="选择协商状态">
                            <el-option label="成功" value="0"></el-option>
                            <el-option label="失败" value="1"></el-option>
                        </el-select>
                    </el-form-item>
                    <el-form-item label="协商关系：">
                        <el-select v-model="form1.consltRelation" clearable placeholder="选择协商状态">
                            <el-option label="主动协商" value="0"></el-option>
                            <el-option label="被动协商" value="1"></el-option>
                        </el-select>
                    </el-form-item>
  ...
  methods: {
    ...
    findOpList() {
      this.$http.post('/api/operator/listOperators?token=' + this.$store.state.user.token, {
        ...
        .then(response => {
          if(response.data.code === 0) {
            this.opList = response.data.data.rows
            return true
          }else{
            return false
          }
    ...
    findData(e) {
      ...
      this.form1.startTime = this.timeList[0].toString()
      this.form1.endTime = this.timeList[1].toString()
      console.log(this.form1)
      let data = qs.stringify(this.form)
      this.$http.post('/api/msisdnMgnt/queryConsultRecordListPd?' + data, this.form1)
        .then(response => {
          console.log(response)
          if(response.data.code === 0) {
